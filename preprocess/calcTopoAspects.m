...
for i = 1:length(flats)
    ...
    if(flats(i).MajorAxisLength/flats(i).MinorAxisLength > parameters.narrowFlatRatio)
        %find the west and south side bounding points
        westPoints =  [round(flats(i).BoundingBox(1)) round(flats(i).BoundingBox(1)+flats(i).BoundingBox(3))];
        southPoints = [round(flats(i).BoundingBox(2)) round(flats(i).BoundingBox(2)+flats(i).BoundingBox(4))];
